...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (session) { showAppScreen(); loadAllData(); } else { showAuthScreen(); }

    _supabase.auth.onAuthStateChange((event, session) => {
        if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') { showAppScreen(); loadAllData(); }
        else if (event === 'SIGNED_OUT') { showAuthScreen(); clearData(); }
    });

    setupAuthHandlers(); setupAppListeners();
});
...
function setupAuthHandlers() {
    const emailInput = document.getElementById('auth-email'), passInput = document.getElementById('auth-pass'), errText = document.getElementById('auth-error');
    function validateInputs() {
        const email = emailInput.value.trim(), pass = passInput.value.trim();
        if (!email || !pass) { errText.textContent = "⚠️ Įveskite el. paštą ir slaptažodį."; errText.classList.remove('hidden'); return false; }
        errText.classList.add('hidden'); return true;
    }
    document.getElementById('btn-login').addEventListener('click', async () => {
        if (!validateInputs()) return;
        const btn = document.getElementById('btn-login'), originalText = btn.innerText;
        btn.innerText = "Jungiama..."; btn.disabled = true;
        try { const { error } = await userLogin(emailInput.value, passInput.value); if (error) throw error; }
        catch (e) { errText.textContent = "Klaida: " + (e.message || 'Prisijungti nepavyko.'); errText.classList.remove('hidden'); btn.innerText = originalText; btn.disabled = false; }
    });
    document.getElementById('btn-signup').addEventListener('click', async () => {
        if (!validateInputs()) return;
        const btn = document.getElementById('btn-signup'), originalText = btn.innerText;
        btn.innerText = "Registruojama..."; btn.disabled = true;
        try { const { error } = await userSignUp(emailInput.value, passInput.value); if (error) throw error; alert("✅ Registracija sėkminga!"); btn.innerText = originalText; btn.disabled = false; }
        catch (e) { errText.textContent = "Klaida: " + (e.message || 'Registracija nepavyko.'); errText.classList.remove('hidden'); btn.innerText = originalText; btn.disabled = false; }
    });
    document.getElementById('btn-logout').addEventListener('click', async () => { if (confirm('Ar tikrai norite atsijungti?')) await userSignOut(); });
}
...
function clearData() {
    ...
    if (investedEl) investedEl.innerText = '$0.00';
...
function setupAppListeners() {
    ...
    if (form) { const newForm = form.cloneNode(true); form.parentNode.replaceChild(newForm, form); newForm.addEventListener('submit', handleTxSubmit); setupCalculator(); }
    const btnSaveCoin = document.getElementById('btn-save-coin');
    if (btnSaveCoin) { btnSaveCoin.replaceWith(btnSaveCoin.cloneNode(true)); document.getElementById('btn-save-coin').addEventListener('click', handleNewCoinSubmit); }
...
function setupCalculator() {
    ...
    amountIn.addEventListener('input', () => { const a = val(amountIn), p = val(priceIn), t = val(totalIn); if (t > 0 && a > 0) { priceIn.value = (t / a).toFixed(8); } else if (p > 0) { totalIn.value = (a * p).toFixed(2); } });
    priceIn.addEventListener('input', () => { const p = val(priceIn), a = val(amountIn), t = val(totalIn); if (t > 0 && p > 0) { amountIn.value = (t / p).toFixed(6); } else if (a > 0) { totalIn.value = (a * p).toFixed(2); } });
...
async function fetchCurrentPrices() {
    ...
    try { const res = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${ids}&vs_currencies=usd`); if (res.ok) { const newPrices = await res.json(); prices = { ...prices, ...newPrices }; console.log('💰 Prices updated'); } }
    catch (e) { console.warn("⚠️ Price fetch error:", e); }
}

async function fetchPriceForForm() {
    const symbol = document.getElementById('tx-coin').value, coin = coinsList.find(c => c.symbol === symbol);
    if (!coin || !coin.coingecko_id) { alert("CoinGecko ID nerastas!"); return; }
    const btn = document.getElementById('btn-fetch-price'), oldText = btn.innerText; btn.innerText = '⏳'; btn.disabled = true;
    try {
        ...
        if (price) { const priceInput = document.getElementById('tx-price'); priceInput.value = price; priceInput.dispatchEvent(new Event('input')); console.log(`✅ Price: ${symbol} = $${price}`); }
...
function updateDashboard() {
    const holdings = {}; let totalInvested = 0;
    ...
    Object.entries(holdings).forEach(([sym, data]) => {
        if (data.qty > 0) {
            ...
            if (coin && prices[coin.coingecko_id]) {
                const currentValue = data.qty * prices[coin.coingecko_id].usd; totalValue += currentValue;
                data.averageBuyPrice = data.totalAmount > 0 ? data.totalCost / data.totalAmount : 0;
                data.currentPrice = prices[coin.coingecko_id].usd; data.currentValue = currentValue;
            }
        }
    });
    const pnl = totalValue - totalInvested, pnlPercent = totalInvested > 0 ? (pnl / totalInvested * 100) : 0;
    ...
    const pnlEl = document.getElementById('total-pnl'); pnlEl.innerText = formatMoney(pnl); pnlEl.style.color = pnl >= 0 ? '#2dd4bf' : '#f87171';
    const pnlPercentEl = document.getElementById('total-pnl-percent'); pnlPercentEl.innerText = (pnl >= 0 ? '+' : '') + pnlPercent.toFixed(2) + '%';
    pnlPercentEl.style.backgroundColor = pnl >= 0 ? '#14532d' : '#7f1d1d'; pnlPercentEl.style.color = pnl >= 0 ? '#34d399' : '#fca5a5';
    return holdings;
}
...
function renderAllocationChart(holdings) {
    ...
    allocationChart = new Chart(ctx, { type: 'doughnut', data: { labels: labels, datasets: [{ data: chartData, backgroundColor: colors, borderColor: '#111827', borderWidth: 2 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { color: '#9ca3af', font: { size: 11 }, padding: 10, usePointStyle: true } }, tooltip: { callbacks: { label: function(context) { const label = context.label || '', value = context.parsed || 0; const total = context.dataset.data.reduce((a, b) => a + b, 0); const percentage = ((value / total) * 100).toFixed(1); return `${label}: ${formatMoney(value)} (${percentage}%)`; } } } } } });
}
...
function renderCoinCards(holdings) {
    ...
    if (activeHoldings.length === 0) { container.innerHTML = '<p class="text-center text-gray-500 text-sm py-4">No active holdings</p>'; return; }
    activeHoldings.forEach(([sym, data]) => {
        ...
        card.innerHTML = `<div class="space-y-4"><div><p class="text-xs text-gray-500 uppercase font-semibold tracking-wide mb-1">Balance</p><h2 class="text-3xl font-bold text-white tracking-tight">${formatMoney(data.currentValue || 0)}</h2><p class="text-sm text-gray-400 mt-1">${data.qty.toFixed(6)} ${sym}</p></div><div class="flex justify-between items-center py-3 border-b border-gray-800"><div class="flex items-center gap-2"><span class="text-sm text-gray-400">Unrealized Return</span><i class="fa-solid fa-arrow-up-right-from-square text-gray-600 text-xs"></i></div><div class="text-right"><p class="${pnlClass} text-base font-bold">${pnlSign}${formatMoney(pnlAmount)} (${pnlSign}${pnlPercent.toFixed(2)}%)</p></div></div><div class="flex justify-between items-center"><span class="text-sm text-gray-400">Average buy price</span><span class="text-base font-semibold text-white">${formatPrice(data.averageBuyPrice || 0)}</span></div><div class="flex justify-between items-center"><span class="text-sm text-gray-400">Cost basis</span><span class="text-base font-semibold text-white">${formatMoney(data.invested || 0)}</span></div></div>`;
...
function renderAccordionJournal() {
    const container = document.getElementById('journal-accordion');
    if (!container) return; container.innerHTML = '';
    if (transactions.length === 0) { container.innerHTML = '<div class="px-4 py-8 text-center text-xs text-gray-600">No transactions yet.</div>'; return; }
    const grouped = {};
    transactions.forEach(tx => { const date = new Date(tx.date), year = date.getFullYear(), month = date.getMonth(); if (!grouped[year]) grouped[year] = {}; if (!grouped[year][month]) grouped[year][month] = []; grouped[year][month].push(tx); });
    const years = Object.keys(grouped).sort((a, b) => b - a);
    years.forEach((year, yearIndex) => {
        const yearDiv = document.createElement('div'); yearDiv.className = 'border border-gray-800 rounded-xl overflow-hidden mb-3';
        const yearHeader = document.createElement('div'); yearHeader.className = 'bg-gray-900 px-4 py-3 flex justify-between items-center cursor-pointer hover:bg-gray-850 transition-colors';
        yearHeader.innerHTML = `<div class="flex items-center gap-2"><i class="fa-solid fa-calendar text-primary-400"></i><span class="font-bold text-white">${year}</span><span class="text-xs text-gray-500">(${Object.values(grouped[year]).flat().length} transactions)</span></div><i class="fa-solid fa-chevron-down text-gray-500 transition-transform year-chevron-${year}"></i>`;
        const monthContainer = document.createElement('div'); monthContainer.id = `year-${year}`; monthContainer.className = yearIndex === 0 ? 'block' : 'hidden';
        ...
        months.forEach((month, monthIndex) => {
            ...
            const monthDiv = document.createElement('div'); monthDiv.className = 'border-t border-gray-800/50';
            const monthHeader = document.createElement('div'); monthHeader.className = 'bg-gray-900/50 px-6 py-2.5 flex justify-between items-center cursor-pointer hover:bg-gray-800/50 transition-colors';
        ...
        yearHeader.addEventListener('click', () => { const isHidden = monthContainer.classList.contains('hidden'); monthContainer.classList.toggle('hidden'); const chevron = yearHeader.querySelector(`.year-chevron-${year}`); if (chevron) { if (isHidden) chevron.style.transform = 'rotate(180deg)'; else chevron.style.transform = 'rotate(0deg)'; } });
        yearDiv.appendChild(yearHeader); yearDiv.appendChild(monthContainer); container.appendChild(yearDiv);
    });
}
...
function renderGoals(holdings) {
    ...
    goals.forEach(goal => {
        ...
        div.innerHTML = `<div class="flex justify-between text-xs mb-1"><span class="font-bold text-gray-300">${goal.coin_symbol}</span><div class="flex items-center gap-2"><span class="text-primary-400 font-bold">${pct.toFixed(1)}%</span><button onclick="onEditGoal('${goal.coin_symbol}', ${target})" class="text-gray-500 hover:text-yellow-500 transition-colors"><i class="fa-solid fa-pen text-[10px]"></i></button></div></div><div class="w-full bg-gray-800 rounded-full h-1.5 overflow-hidden"><div class="bg-primary-500 h-1.5 rounded-full transition-all duration-500" style="width: ${pct}%"></div></div><div class="text-[9px] text-gray-500 mt-1 text-right font-mono">${current.toLocaleString(undefined, {maximumFractionDigits: 2})} / ${target.toLocaleString()}</div>`;
...
function showCelebration(symbol) {
    ...
    document.getElementById('celebration-coin').innerText = symbol;
...
window.onEditGoal = function(coinSymbol, currentTarget) {
    ...
    if (isNaN(target) || target <= 0) { alert('Invalid target amount!'); return; }
    saveOrUpdateGoal(coinSymbol, target).then(success => { if (success) { loadAllData(); } });
};

async function generateHistoryChart() {
    if (transactions.length === 0) { renderChart(['No data'], [0]); return; }
    const dates = transactions.map(t => new Date(t.date).getTime()), minDate = new Date(Math.min(...dates)), maxDate = new Date();
    ...
    for (let d = new Date(minDate); d <= maxDate; d.setDate(d.getDate() + 1)) {
        ...
        transactions.forEach(tx => { const txDate = new Date(tx.date).toISOString().split('T')[0]; if (txDate <= dateStr) { 
            if (['Buy', 'Instant Buy', 'Recurring Buy', 'Limit Buy', 'Market Buy', 'Staking Reward', 'Bonus', 'Gift/Airdrop'].includes(tx.type)) balances[tx.coin_symbol] = (balances[tx.coin_symbol] || 0) + Number(tx.amount);
            else if (tx.type === 'Sell' || tx.type === 'Withdraw') balances[tx.coin_symbol] = (balances[tx.coin_symbol] || 0) - Number(tx.amount);
        } });
        for (const [sym, qty] of Object.entries(balances)) { if (qty > 0) { const coin = coinsList.find(c => c.symbol === sym); if (coin && prices[coin.coingecko_id]) dailyValue += qty * prices[coin.coingecko_id].usd; } }
...
function renderChart(labels, data) {
    ...
    const ctx = ctxEl.getContext('2d'), grad = ctx.createLinearGradient(0, 0, 0, 160);
    ...
    myChart = new Chart(ctx, { type: 'line', data: { labels: labels, datasets: [{ data: data, borderColor: borderColor, backgroundColor: grad, borderWidth: 2, fill: true, tension: 0.3, pointRadius: 0, pointHitRadius: 10 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { mode: 'index', intersect: false, callbacks: { label: (ctx) => formatMoney(ctx.parsed.y) } } }, scales: { x: { display: false }, y: { display: false } } } });
}
...
async function handleDeleteCoinSubmit() {
    ...
    if (!sym) { alert("Pasirinkite monetą!"); return; }
